import { Pressable, View, type StyleProp, type ViewStyle } from "react-native";
import { useRouter } from "expo-router";
import { Clock, Users, Sparkles, BadgeCheck } from "lucide-react-native";
import { colors, radius, shadows } from "@/theme";
import { FARE_MODE_LABELS, RIDE_STATUS_LABELS, type Ride } from "@/types/ride";
import { Avatar } from "@/components/ui/Avatar";
import { AppText } from "@/components/ui/AppText";
import { Chip } from "@/components/ui/Chip";
import { Shimmer } from "@/components/ui/animations";
import { Rating } from "./Badges";
import { RouteLine } from "./RouteLine";

const naira = (n: number) => `₦${n.toLocaleString()}`;

function formatDeparture(iso: string): { date: string; time: string } {
  const d = new Date(iso);
  const today = new Date();
  const sameDay = (a: Date, b: Date) =>
    a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate();
  const date = sameDay(d, today)
    ? "Today"
    : d.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" });
  const time = d.toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit", hour12: false });
  return { date, time };
}

export function RideCard({
  ride,
  style,
  onPress,
}: {
  ride: Ride;
  style?: StyleProp<ViewStyle>;
  onPress?: () => void;
}) {
  const router = useRouter();
  const hostName = ride.hostDisplayName ?? "Covia host";
  const initials = hostName
    .split(" ")
    .map((n) => n[0])
    .join("")
    .slice(0, 2);
  const { date, time } = formatDeparture(ride.departureTime);
  const tags = [
    ride.isWomenOnly ? "Women only" : null,
    ride.isStudentOnly ? "Students" : null,
  ].filter((t): t is string => t !== null);
  const active = ride.rideStatus === "published" || ride.rideStatus === "full";

  return (
    <Pressable
      onPress={onPress ?? (() => router.push(`/ride/${ride.id}`))}
      style={({ pressed }) => [
        {
          borderRadius: radius["2xl"],
          borderWidth: 1,
          borderColor: colors.border,
          backgroundColor: colors.card,
          padding: 16,
          ...shadows.soft,
          opacity: pressed ? 0.96 : 1,
          transform: [{ scale: pressed ? 0.99 : 1 }],
        },
        style,
      ]}
    >
      <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
        <Avatar
          src={ride.hostAvatarUrl ?? undefined}
          fallback={initials}
          size={44}
          ring={{ color: colors.primarySoft }}
        />
        <View style={{ flex: 1, minWidth: 0 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            <AppText size="sm" weight={600} numberOfLines={1} style={{ flexShrink: 1 }}>
              {hostName}
            </AppText>
            {ride.hostRating != null ? <Rating value={ride.hostRating} /> : null}
          </View>
          {ride.hostVerified ? (
            <View style={{ marginTop: 4, flexDirection: "row", alignItems: "center", gap: 4 }}>
              <BadgeCheck size={12} color={colors.primary} strokeWidth={2.5} />
              <AppText size="xs" weight={600} color={colors.primary} style={{ fontSize: 10 }}>
                ID verified
              </AppText>
            </View>
          ) : null}
        </View>
        <View style={{ alignItems: "flex-end" }}>
          {ride.fareMode === "fixed" && ride.fixedFare != null ? (
            <>
              <AppText size="base" family="display" weight={700} style={{ lineHeight: 20 }}>
                {naira(ride.fixedFare)}
              </AppText>
              <AppText size="xs" color={colors.mutedForeground} style={{ fontSize: 10, marginTop: 4 }}>
                per seat
              </AppText>
            </>
          ) : (
            <Chip style={{ paddingVertical: 4 }}>
              <AppText size="xs" weight={600} color={colors.primary}>
                Smart fare
              </AppText>
            </Chip>
          )}
        </View>
      </View>

      <View style={{ height: 1, backgroundColor: colors.border, marginVertical: 12 }} />

      <RouteLine
        pickup={ride.pickupPoint || ride.origin}
        destination={ride.destination}
        landmark={ride.pickupPointLoc?.full_address ?? undefined}
      />

      <View
        style={{
          marginTop: 12,
          flexDirection: "row",
          flexWrap: "wrap",
          alignItems: "center",
          columnGap: 16,
          rowGap: 8,
        }}
      >
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          <Clock size={14} color={colors.mutedForeground} />
          <AppText size="xs" color={colors.mutedForeground}>
            {date} · {time}
          </AppText>
        </View>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          <Users size={14} color={colors.mutedForeground} />
          <AppText size="xs" color={colors.mutedForeground}>
            {ride.availableSeats > 0
              ? `${ride.availableSeats} of ${ride.totalSeats} seats left`
              : "Full"}
          </AppText>
        </View>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          <Sparkles size={14} color={colors.mutedForeground} />
          <AppText size="xs" color={colors.mutedForeground}>
            {FARE_MODE_LABELS[ride.fareMode]}
          </AppText>
        </View>
        {!active ? (
          <Chip active={ride.rideStatus === "in_progress"} style={{ paddingVertical: 2 }}>
            {RIDE_STATUS_LABELS[ride.rideStatus]}
          </Chip>
        ) : null}
      </View>

      {tags.length ? (
        <View style={{ marginTop: 12, flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
          {tags.map((t) => (
            <Chip key={t} style={{ paddingVertical: 4 }}>
              {t}
            </Chip>
          ))}
        </View>
      ) : null}
    </Pressable>
  );
}

export function RideCardSkeleton({ style }: { style?: StyleProp<ViewStyle> }) {
  return (
    <View style={[{ borderRadius: radius["2xl"], borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, padding: 16 }, style]}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
        <Shimmer width={44} height={44} />
        <View style={{ flex: 1, gap: 8 }}>
          <Shimmer width={128} height={12} />
          <Shimmer width={96} height={12} />
        </View>
      </View>
      <View style={{ marginTop: 16, gap: 8 }}>
        <Shimmer width="100%" height={12} radius={6} />
        <Shimmer width="66%" height={12} radius={6} />
      </View>
    </View>
  );
}
